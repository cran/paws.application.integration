...
#' @param AWSAccountIds &#91;required&#93; The Amazon Web Services account numbers of the [principals](https://docs.aws.amazon.com/glossary/latest/reference/glos-chap.html#P) who are to receive permission. For information about locating the Amazon Web Services account identification, see [Your Amazon Web Services Identifiers](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-making-api-requests-xml.html#sqs-api-request-authentication) in the *Amazon SQS Developer Guide*.
...
#' 
#' Specifying [`send_message`][sqs_send_message], [`delete_message`][sqs_delete_message], or [`change_message_visibility`][sqs_change_message_visibility] for `ActionName.n` also grants permissions for the corresponding batch versions of those actions: [`send_message_batch`][sqs_send_message_batch], [`delete_message_batch`][sqs_delete_message_batch], and [`change_message_visibility_batch`][sqs_change_message_visibility_batch].
#'
#' @keywords internal
#'
...
#' 
#' Queue URLs and names are case-sensitive.
#' @param ReceiptHandle &#91;required&#93; The receipt handle associated with the message, whose visibility timeout is changed. This parameter is returned by the [`receive_message`][sqs_receive_message] action.
#' @param VisibilityTimeout &#91;required&#93; The new value for the message's visibility timeout (in seconds). Values range: `0` to `43200`. Maximum: 12 hours.
#'
#' @keywords internal
#'
...
#' 
#' Queue URLs and names are case-sensitive.
#' @param Entries &#91;required&#93; Lists the receipt handles of the messages for which the visibility timeout must be changed.
#'
#' @keywords internal
#'
...
#' 
#' The following lists the names, descriptions, and values of the special request parameters that the [`create_queue`][sqs_create_queue] action uses:
#' 
...
#' 
#' -   `Policy` – The queue's policy. A valid Amazon Web Services policy. For more information about policy structure, see [Overview of Amazon Web Services IAM Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html) in the *IAM User Guide*.
#' 
#' -   `ReceiveMessageWaitTimeSeconds` – The length of time, in seconds, for which a [`receive_message`][sqs_receive_message] action waits for a message to arrive. Valid values: An integer from 0 to 20 (seconds). Default: 0.
#' 
...
#' 
#' The following attributes apply only to [FIFO (first-in-first-out) queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/):
#' 
...
#' @param tags Add cost allocation tags to the specified Amazon SQS queue. For an overview, see [Tagging Your Amazon SQS Queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-queue-tags.html) in the *Amazon SQS Developer Guide*.
#' 
...
#' 
#' For a full list of tag restrictions, see [Quotas related to queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-quotas.html#limits-queues) in the *Amazon SQS Developer Guide*.
#' 
...
#' 
#' -   `CreatedTimestamp` – Returns the time when the queue was created in seconds ([epoch time](https://en.wikipedia.org/wiki/Unix_time)).
#' 
#' -   `DelaySeconds` – Returns the default delay on the queue in seconds.
#' 
#' -   `LastModifiedTimestamp` – Returns the time when the queue was last changed in seconds ([epoch time](https://en.wikipedia.org/wiki/Unix_time)).
#' 
...
#' 
#' -   `ReceiveMessageWaitTimeSeconds` – Returns the length of time, in seconds, for which the [`receive_message`][sqs_receive_message] action waits for a message to arrive.
#' 
...
#' 
#'     To determine whether a queue is [FIFO](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/), you can check whether `QueueName` ends with the `.fifo` suffix.
#' 
...
#'
#' @param QueueName &#91;required&#93; (Required) The name of the queue for which you want to fetch the URL. The name can be up to 80 characters long and can include alphanumeric characters, hyphens (-), and underscores (_). Queue URLs and names are case-sensitive.
#' @param QueueOwnerAWSAccountId (Optional) The Amazon Web Services account ID of the account that created the queue. This is only required when you are attempting to access a queue owned by another Amazon Web Services account.
#'
#' @keywords internal
#'
...
#' @param MaxResults The maximum number of results to include in the response. The default is 1, which provides the most recent message movement task. The upper limit is 10.
#'
#' @keywords internal
#'
...
#'
#' @param QueueUrl &#91;required&#93; The URL of the queue from which the [`purge_queue`][sqs_purge_queue] action deletes messages.
#' 
#' Queue URLs and names are case-sensitive.
#'
#' @keywords internal
...
#' 
#' Queue URLs and names are case-sensitive.
#' @param AttributeNames This parameter has been discontinued but will be supported for backward compatibility. To provide attribute names, you are encouraged to use `MessageSystemAttributeNames`.
#' 
#' A list of attributes that need to be returned along with each message. These attributes include:
#' 
#' -   `All` – Returns all values.
#' 
#' -   `ApproximateFirstReceiveTimestamp` – Returns the time the message was first received from the queue ([epoch time](https://en.wikipedia.org/wiki/Unix_time) in milliseconds).
#' 
...
#' 
#' -   `SentTimestamp` – Returns the time the message was sent to the queue ([epoch time](https://en.wikipedia.org/wiki/Unix_time) in milliseconds).
#' 
#' -   `SqsManagedSseEnabled` – Enables server-side queue encryption using SQS owned encryption keys. Only one server-side encryption option is supported per queue (for example, [SSE-KMS](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-configure-sse-existing-queue.html) or [SSE-SQS](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-configure-sqs-sse-queue.html)).
#' 
#' -   `MessageDeduplicationId` – Returns the value provided by the producer that calls the [`send_message`][sqs_send_message] action.
#' 
#' -   `MessageGroupId` – Returns the value provided by the producer that calls the [`send_message`][sqs_send_message] action.
#' 
...
#' 
#' When using [`receive_message`][sqs_receive_message], you can send a list of attribute names to receive, or you can return all of the attributes by specifying `All` or `.*` in your request. You can also use all message attributes starting with a prefix, for example `bar.*`.
...
#' @param VisibilityTimeout The duration (in seconds) that the received messages are hidden from subsequent retrieve requests after being retrieved by a [`receive_message`][sqs_receive_message] request. If not specified, the default visibility timeout for the queue is used, which is 30 seconds.
#' 
...
#' 
#' To avoid HTTP errors, ensure that the HTTP response timeout for [`receive_message`][sqs_receive_message] requests is longer than the `WaitTimeSeconds` parameter. For example, with the Java SDK, you can set HTTP transport settings using the [NettyNioAsyncHttpClient](https://docs.aws.amazon.com/java/api/latest/software/amazon/awssdk/http/nio/netty/NettyNioAsyncHttpClient.html) for asynchronous clients, or the [ApacheHttpClient](https://docs.aws.amazon.com/java/api/latest/software/amazon/awssdk/http/apache/ApacheHttpClient.html) for synchronous clients.
#' @param ReceiveRequestAttemptId This parameter applies only to FIFO (first-in-first-out) queues.
#' 
#' The token used for deduplication of [`receive_message`][sqs_receive_message] calls. If a networking issue occurs after a [`receive_message`][sqs_receive_message] action, and instead of a response you receive a generic error, it is possible to retry the same action with an identical `ReceiveRequestAttemptId` to retrieve the same set of messages, even if their visibility timeout has not yet expired.
#' 
#' -   You can use `ReceiveRequestAttemptId` only for 5 minutes after a [`receive_message`][sqs_receive_message] action.
#' 
#' -   When you set `FifoQueue`, a caller of the [`receive_message`][sqs_receive_message] action can provide a `ReceiveRequestAttemptId` explicitly.
#' 
#' -   It is possible to retry the [`receive_message`][sqs_receive_message] action with the same `ReceiveRequestAttemptId` if none of the messages have been modified (deleted or had their visibility changes).
#' 
...
#' 
#' -   If a caller of [`receive_message`][sqs_receive_message] can't track the `ReceiveRequestAttemptId`, no retries work until the original visibility timeout expires. As a result, delays might occur but the messages in the queue remain in a strict order.
#' 
...
#' 
#' Queue URLs and names are case-sensitive.
#' @param Label &#91;required&#93; The identification of the permission to remove. This is the label added using the [`add_permission`][sqs_add_permission] action.
#'
#' @keywords internal
#'
...
#' 
#' Queue URLs and names are case-sensitive.
#' @param MessageBody &#91;required&#93; The message to send. The minimum size is one character. The maximum size is 1 MiB or 1,048,576 bytes
#' 
#' A message can include only XML, JSON, and unformatted text. The following Unicode characters are allowed. For more information, see the [W3C specification for characters](https://www.w3.org/TR/REC-xml/#charsets).
#' 
...
#' @param MessageAttributes Each message attribute consists of a `Name`, `Type`, and `Value`. For more information, see [Amazon SQS message attributes](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-message-metadata.html#sqs-message-attributes) in the *Amazon SQS Developer Guide*.
...
#' 
#'     [`receive_message`][sqs_receive_message] might return messages with multiple `MessageGroupId` values. For each `MessageGroupId`, the messages are sorted by time sent.
#' 
...
#' 
#' The following lists the names, descriptions, and values of the special request parameters that the [`set_queue_attributes`][sqs_set_queue_attributes] action uses:
#' 
...
#' 
#' -   `Policy` – The queue's policy. A valid Amazon Web Services policy. For more information about policy structure, see [Overview of Amazon Web Services IAM Policies](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html) in the *Identity and Access Management User Guide*.
#' 
#' -   `ReceiveMessageWaitTimeSeconds` – The length of time, in seconds, for which a [`receive_message`][sqs_receive_message] action waits for a message to arrive. Valid values: An integer from 0 to 20 (seconds). Default: 0.
#' 
#' -   `VisibilityTimeout` – The visibility timeout for the queue, in seconds. Valid values: An integer from 0 to 43,200 (12 hours). Default: 30. For more information about the visibility timeout, see [Visibility Timeout](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-visibility-timeout.html) in the *Amazon SQS Developer Guide*.
...
#' -   `SqsManagedSseEnabled` – Enables server-side queue encryption using SQS owned encryption keys. Only one server-side encryption option is supported per queue (for example, [SSE-KMS](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-configure-sse-existing-queue.html) or [SSE-SQS](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-configure-sqs-sse-queue.html)).
#' 
#' The following attribute applies only to [FIFO (first-in-first-out) queues](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/):
#' 
...
#'
#' @param SourceArn &#91;required&#93; The ARN of the queue that contains the messages to be moved to another queue. Currently, only ARNs of dead-letter queues (DLQs) whose sources are other Amazon SQS queues are accepted. DLQs whose sources are non-SQS queues, such as Lambda or Amazon SNS topics, are not currently supported.
#' @param DestinationArn The ARN of the queue that receives the moved messages. You can use this field to specify the destination queue where you would like to redrive messages. If this field is left blank, the messages will be redriven back to their respective original source queues.
